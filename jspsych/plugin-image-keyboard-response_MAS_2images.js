var jsPsychImageKeyboardResponseMAS2images = (function (jspsych) {
  'use strict';

  var version = "2.1.0";

  const info = {
    name: "image-keyboard-response_MAS_2images",
    version,
    parameters: {
  ...
  class ImageKeyboardResponsePlugin {
    ...
    trial(display_element, trial) {
      var heightLeft, widthLeft;
      var heightRight, widthRight;
      if (trial.render_on_canvas) {
        ...
        imgLeft.onload = () => {
          if (!image_drawn) {
            getHeightWidth();
            ctx.drawImage(imgLeft, 0, 0, widthLeft, heightLeft);
          }
        };
        imgRight.onload = () => {
          if (!image_drawn) {
            getHeightWidth();
            ctx.drawImage(imgRight, 0, 0, widthRight, heightRight);
          }
        };

        imgLeft.src = trial.stimulusLeft;
        imgRight.src = trial.stimulusRight;

        const getHeightWidth = () => {
          if (trial.stimulusL_height !== null) {
            heightLeft = trial.stimulusL_height;
            heightRight = trial.stimulusR_height;
            if (trial.stimulusL_width == null && trial.maintain_aspect_ratio) {
              widthLeft = imgLeft.naturalWidth * (trial.stimulusL_height / imgLeft.naturalHeight);
              widthRight = imgRight.naturalWidth * (trial.stimulusR_height / imgRight.naturalHeight);

            }
          } else {
            heightLeft = imgLeft.naturalHeight;
             heightRight = imgRight.naturalHeight;

          }
          if (trial.stimulusL_width !== null) {
            widthLeft = trial.stimulusL_width;
            widthRight = trial.stimulusR_width;

            if (trial.stimulusL_height == null && trial.maintain_aspect_ratio) {
              heightLeft = imgLeft.naturalHeight * (trial.stimulusL_width / imgLeft.naturalWidth);
              heightRight = imgRight.naturalHeight * (trial.stimulusR_width / imgRight.naturalWidth);

            }
          } else if (!(trial.stimulusL_heightL !== null && trial.maintain_aspect_ratio)) {
            widthLeft = imgLeft.naturalWidth;
            widthRight = imgRight.naturalWidth;

          }
          canvas.height = heightLeft;
          canvas.width = widthLeft*2;
        };

        getHeightWidth();
        display_element.insertBefore(canvas, null);
        if (imgLeft.complete && Number.isFinite(widthLeft) && Number.isFinite(heightLeft)) {
          ctx.drawImage(imgLeft, 0, 0, widthLeft, heightLeft);
          image_drawn = true;
        }
      ...
      if (imgRight.complete && Number.isFinite(widthRight) && Number.isFinite(heightRight)) {
          ctx.drawImage(imgRight, 0, 0, widthRight, heightRight);
          image_drawn = true;
        }
        if (trial.prompt !== null) {
      ...
      //not on canvas
       ...
       var html = '<img src="' + trial.stimulusLeft + '" id="jspsych-image-keyboard-response-stimulusLeft">';
           html += '<img src="' + trial.stimulusRight + '" id="jspsych-image-keyboard-response-stimulusRight">';

       if (trial.prompt !== null) {
          html += trial.prompt;
        }
        display_element.innerHTML = html;
        var imgLeft = display_element.querySelector(
          "#jspsych-image-keyboard-response-stimulusLeft"
        );
         var imgRight = display_element.querySelector(
          "#jspsych-image-keyboard-response-stimulusRight"
        );
        ...
        } else if (!(trial.stimulusL_height !== null && trial.maintain_aspect_ratio)) {
          widthLeft = imgLeft.naturalWidth;
          widthRight = imgRight.naturalWidth;

        }
        imgLeft.style.height= heightLeft.toString() + "px";
        imgLeft.style.width = widthLeft.toString() + "px";
        imgLeft.style.position = "relative"; 
        imgLeft.style.top = trial.stimulusL_top + "px" ;
        imgLeft.style.left = trial.stimulusL_left + "px";

        imgRight.style.height = heightRight.toString() + "px";
        imgRight.style.width= widthRight.toString() + "px";
        imgRight.style.position = "relative"; 
        imgRight.style.top = trial.stimulusR_top + "px" ;
        imgRight.style.left = trial.stimulusR_left + "px";
      }
      ...
      const end_trial = () => {
        ...
        var trial_data = {
          rt: response.rt,
          stimulusLeft: trial.stimulusLeft,
          stimulusRight: trial.stimulusRight,
          response: response.key
        };
        this.jsPsych.finishTrial(trial_data);
      };
      // var after_response = (info2) => {
      //   display_element.querySelector("#jspsych-image-keyboard-response-stimulusLeft").className += " responded";
      ...
      // };
          ...
          var after_response = (info2) => {
            // Add responded class to BOTH left and right stimuli
            let leftStim = display_element.querySelector("#jspsych-image-keyboard-response-stimulusLeft");
            let rightStim = display_element.querySelector("#jspsych-image-keyboard-response-stimulusRight");

            if (leftStim) leftStim.classList.add("responded");
            if (rightStim) rightStim.classList.add("responded");

            // Only set response if it hasn't been recorded
            if (response.key === null) {
              //response = info2; 
              response.key = info2.key;   // <-- just the string (e.g., "ArrowLeft")
              response.rt = info2.rt;  // info2 should have {key: "ArrowLeft", rt: ...}
            }
